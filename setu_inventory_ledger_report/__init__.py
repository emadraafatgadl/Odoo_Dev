            ...
            From  (
                Select * from (
                    Select  
                        ...
                        0 as opening_stock,
                        case when source.usage = 'internal' and dest.usage = 'customer' then move.qty_done else 0 end as sales, 
                        case when source.usage = 'customer' and dest.usage = 'internal' then move.qty_done else 0 end as sales_return,
                        case when source.usage = 'supplier' and dest.usage = 'internal' then move.qty_done else 0 end as purchase,
                        case when source.usage = 'internal' and dest.usage = 'supplier' then move.qty_done else 0 end as purchase_return,
                        case when source.usage = 'internal' and dest.usage = 'inventory' then move.qty_done else 0 end as adjustment_out,
                        case when source.usage = 'inventory' and dest.usage = 'internal' then move.qty_done else 0 end as adjustment_in,
                        case when source.usage = 'internal' and dest.usage = 'production' then move.qty_done else 0 end as production_out,
                        case when source.usage = 'production' and dest.usage = 'intenal' then move.qty_done else 0 end as production_in,
                        case when source.usage = 'internal' and dest.usage = 'transit' then move.qty_done else 0 end as transit_out,
                        case when source.usage = 'transit' and dest.usage = 'internal' then move.qty_done else 0 end as transit_in,
                        0 as internal_out,
                        0 as internal_in
