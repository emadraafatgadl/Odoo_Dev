...
class SetuInventoryLedger(models.TransientModel):
    ...
    # Fetching companies set at user level.
    @api.model
    def _get_company_ids(self):
        user = self.env.user
        return [('id', 'in', user.company_ids.ids)]
    ...
    company_ids = fields.Many2many("res.company", string="Companies", domain=_get_company_ids)
    ...
    def download_report_in_listview(self):
        ...
        # print(len(stock_data),"lllll")
        for ledger_data_value in stock_data:
            # print(ledger_data_value.keys(), "ledger_data_value")
            # ledger_data_value['wizard_id'] = self.id
            sql = """INSERT INTO setu_inventory_ledger_bi_report(wizard_id, company_id, product_id, product_category_id, inventory_date, opening_stock, sales, purchase, sales_return, purchase_return, adjustment_in, adjustment_out, production_in, production_out, closing)
                     VALUES ('%s', '%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')""" % (
                         self.id,
                         ledger_data_value['company_id'],
                         ledger_data_value['product_id'],
                         ledger_data_value['product_category_id'],
                         ledger_data_value['inventory_date'],
                         ledger_data_value['opening_stock'],
                         ledger_data_value['sales'],
                         ledger_data_value['purchase'],
                         ledger_data_value['sales_return'],
                         ledger_data_value['purchase_return'],
                         ledger_data_value['adjustment_in'],
                         ledger_data_value['adjustment_out'],
                         ledger_data_value['production_in'],
                         ledger_data_value['production_out'],
                         ledger_data_value['closing']
                     )
            print(sql,"slll")
...
class SetuInventoryLedgerBIReport(models.TransientModel):
    ...
    ledger_detail_ids = fields.Many2many("stock.move.line", string="Product movements", compute='_calculate_ledger_details')
    ...
    def _calculate_ledger_details(self):
        ...
        move_ids = self.env['stock.move.line'].search(domain + domain1).ids
        if move_ids:
